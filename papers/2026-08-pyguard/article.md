...
![图1：左边是恶意代码，读取系统密码文件、做 base64 编码、再通过 urllib 把数据发到一个外部收集地址，一条典型的信息外泄链；右边是正常代码，一个图片处理函数，同样用了 urllib 拼 URL、用 base64 编码图片。两段代码在 API 层面高度相似（都碰了敏感路径、网络连接、编码），于是所有主流工具都把右边这个正常包误报成了恶意](assets/fig/fig1-motivating.png)
