...
- `cover_stat:` B 版数据块里的数字。**优先写成对比**，如 `13.9% → 62.5%`、`85.2% -> 24.0%`；
  只有一个值时直接写 `45.83%`，箭头会自动隐藏。字号按长度自动缩放，不用自己调。
- `cover_stat_label:` 这组数字**是什么**，如"GPT-5.4 攻击成功率"。和 `cover_stat` 一起构成
  B 版的数据块，**两个都填才有资格走 B 版**，缺一个就退回 A。这条是硬约束：
  早期版本让一个孤零零的大数字占满半张封面，结果既没有语境（读者不知道是什么的数），
  又和旁边那句结论重复（结论里已经念了一遍这个数），是设计上的双重浪费。
...
| **B 数据对比** | 左边带标签的数据块（从 X 到 Y），右边一句**和数字互补**的结论，底部两条看点 + 会议徽章 | 有一组说得清的数字时 |

- 会议报告不走这两套，`kind: talk` 自动套 `templates/cover_talk.html`（浅色底 + 左侧竖排色条），
  见 TALK_WORKFLOW.md。
- **B 版的 `cover_title` 不要复述数字**，数据块已经把数说了，这句要接着往下讲机制或后果。
