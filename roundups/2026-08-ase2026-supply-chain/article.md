---
title: ASE 2026 接收名单里的供应链安全：12 篇论文，从恶意包一路排到智能体技能生态
title_en: "ASE 2026 Accepted Papers on Software Supply Chain Security"
kind: roundup
cover_title: 从恶意包到智能体技能，供应链安全在 ASE 2026 铺开五条线
cover_stat: "12 篇"
cover_stat_label: ASE 2026 供应链相关论文
digest: ASE 2026 接收名单公布，我们把与软件供应链安全相关的论文挑出来做了一期盘点：12 篇，覆盖包与生态治理、智能体供应链、依赖与构建、漏洞与补丁传播、提交与发布防线五个方向，每篇都给了标题、单位和一句话导读。
author: 供应链安全前沿
venue: ASE 2026
collection: 供应链攻击与防御
highlights:
...
【来源】ASE 2026 官方接收论文列表（研究轨与 Industry Showcase 轨）
...
其一，**这份名单是我逐篇核过的，但不敢说全**。ASE 2026 研究轨接收了 200 多篇，官方页面只给标题和作者名、不给单位，我是把接收列表按字母顺序整个过了一遍再自己判断相关性，不是丢几个关键词让工具筛。但抓取受限，字母 Q 到 Z 那一段我没能完整拿到，==所以这一期确定覆盖 A 到 P，后半段可能有遗漏==，欢迎在留言区补。

其二，**多数论文全文还没公开**，ASE 要到十月才开会。所以下面每篇的介绍分两种：本账号已经读过全文的，写的是实际内容；其余的，介绍严格限制在标题和作者团队背景能支撑的范围内，不替作者把结论说满。凡是我核不到单位的，宁可不写进正表。
...
下面这些从标题看也属于供应链安全的范畴，但我没能核实到作者单位（researchr 的作者主页有的设了私密，有的重名对不上），按只写核实过的原则没进正表，标题先列在这里供检索：

- Context-Aware Trust Verification for Identity-Based Software Signing（软件签名的信任验证）
- Guided Decoding as a Defense against Package Hallucination in LLM-Generated Code（防包幻觉）
- A Longitudinal Study of Android Apps Signing Key Protection（签名密钥保护的纵向研究）
- Defensive Capability Analysis for JavaScript Libraries（JS 库的防御能力分析）
- Execution-as-Configuration: Security Smells in Model Configuration Artifacts（模型配置制品里的安全坏味）
- Assessing the Cross-Version Applicability of Java Library Vulnerability Exploits（Java 库漏洞利用的跨版本适用性）
- Learning to Triage Vulnerability Reports from Program Analysis: An Empirical Study in Node.js（Node.js 漏洞报告分诊）
- "Impossible to Hide Secret ...": Uncovering Security and Privacy Issues in LLM-Native IDEs（AI 原生 IDE 的安全与隐私问题）

Industry Showcase 轨里也有几篇沾边的，一并记下：DepWareTrans（依赖感知的跨语言仓库迁移）、AgenticSCR（自主智能体做安全代码审查）、Vulnerability Tracking using Normalized Scope+Offset（漏洞追踪）、An Empirical Study of Security Risks in the Android Push Notification Ecosystem（安卓推送生态的安全风险）。

## TAKEAWAYS|一点观察

**智能体供应链已经单独成一支了。** 三篇 Agent Skill 相关的论文进入 ASE 主轨，而且分工很清楚：一篇做恶意检测，一篇做生态级克隆识别，一篇做凭据泄露实证。这个结构和十年前包生态刚起步时几乎一模一样，先有人发现恶意样本，再有人做规模化测量，然后才是检测工具和治理机制。==技能生态正在用比包生态快得多的速度，重走同一条路==。

**"修复传不到底"是这批论文最集中的母题。** 补丁回移、一个漏洞多个补丁、依赖升级自动化、后量子迁移就绪度，四篇讲的其实是同一件事的不同切面：漏洞的修复往往是存在的，真正的风险在于它到不了所有下游。这个判断从数据上比"某某组件有漏洞"要更有行动价值，因为它指向的是流程和工具，而不是某一个具体的洞。

**方法论上有个共同转向：从"检测单个制品"转向"理解整个生态的结构"。** 复用谱系、生态分类体系、隐式依赖、克隆关系，这几篇都不是在判断某个包是好是坏，而是在还原"谁来自谁、谁包含谁"。这种结构性知识才是风险传播分析的底座，也是目前最缺的基础设施。

**最后一句关于团队分布**：南开大学和南洋理工大学的团队在这批里出现得最密集，多篇之间还互为合作。国内做软件供应链安全的力量已经在顶会形成了稳定的输出，这在几年前还看不到。
